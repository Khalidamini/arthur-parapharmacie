...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Vérifier le rôle admin ou pharmacy
    ...
    if (!roles || !['admin', 'owner', 'admin'].includes(roles.role)) {
...
function generatePythonConnector(supabaseUrl: string): string {
...
class ArthurConnector:
    ...
    def sync(self):
        ...
        if not self.config.is_configured():
            ...
            print("Lancez: python arthur-connector.py --configure\\n")
            return False
